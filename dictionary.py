#dictionary
person = {"age": 18, "name": "Chad", "country": "Germany"}
...
for m in students:
    total = m["maths"] + m["english"] + m["kiswahili"] + m["chemistry"] + m["biology"] + m["accounting"] + m["history"]
    m["total"] = total

# print(students[18])
sorted_by_eng = sorted(students, key = lambda x : x["english"], reverse=True)#descending=True ascending=False

sorted_by_total = sorted(students, key = lambda x : x["total"], reverse=True)
# for m in sorted_by_eng:
#      print(m["english"])
...
#top Female
    for k in sorted_by_total:
        if k["gender"] == "Female":
            print(k["names"], k["total"])
            break

#top Male in Chemistry
sorted_by_chem = sorted(students, key = lambda x : x["chemistry"], reverse=True)
for y in sorted_by_chem:
    if y["gender"] == "Male":
        print(y["names"], y["chemistry"])
        break

sorted_by_eng = sorted(students, key = lambda x : x["english"], reverse=False)
for r in sorted_by_eng:
    if r["gender"] == "Female":

        print(r["names"], r["english"])
        break